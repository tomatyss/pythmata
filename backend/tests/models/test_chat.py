...
@pytest.fixture
async def chat_session(session: AsyncSession, process_definition: ProcessDefinition) -> ChatSession:
...
async def test_chat_session_creation(session: AsyncSession, process_definition: ProcessDefinition):
...
async def test_chat_session_relationship(
    ...
    assert messages[1].content == "I'd be happy to help! What specific aspects of your BPMN process do you need assistance with?"
    assert messages[2].content == "I need to add a service task."


async def test_process_definition_relationship(
    session: AsyncSession, process_definition: ProcessDefinition, chat_session: ChatSession
):
...
async def test_chat_message_with_xml_content(session: AsyncSession, chat_session: ChatSession):
...
async def test_cascade_delete(session: AsyncSession, chat_session: ChatSession, chat_messages: list[ChatMessage]):
    ...
    for message in chat_messages:
        result = await session.execute(select(ChatMessage).where(ChatMessage.id == message.id))
        assert result.scalars().first() is None


async def test_chat_session_updated_at(session: AsyncSession, chat_session: ChatSession):
...
async def test_chat_session_ordering(session: AsyncSession, process_definition: ProcessDefinition):
