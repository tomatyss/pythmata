...
from pythmata.core.engine.executor import ProcessGraphValidationError
...
class TestProcessGraphValidation(BaseEngineTest):
    ...
    async def test_invalid_graph_missing_flows(self):
        ...
        invalid_graph = {"nodes": [
            Event(id="Start_1", type="event", event_type=EventType.START, outgoing=[])
        ]}
    ...
    async def test_missing_start_event(self):
        ...
        graph = {
            ...
            "flows": [
                {"id": "Flow_1", "source_ref": "Task_1", "target_ref": "End_1"}
            ]
        }
