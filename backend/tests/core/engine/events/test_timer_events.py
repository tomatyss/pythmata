...
class TestTimerEvents:
    ...
    async def test_timer_duration_execution(self):
        ...
        token = Token(instance_id="test-duration-1", node_id="Timer_1", data={})
    ...
    async def test_timer_cancellation(self):
        ...
        timer = TimerEvent("Timer_1", "PT1H", self.state_manager)
        token = Token(instance_id="test-cancel-1", node_id="Timer_1", data={})
    ...
    async def test_timer_state_persistence(self):
        ...
        timer = TimerEvent("Timer_1", "PT1H", self.state_manager)
        token = Token(
            instance_id="test-persist-1", node_id="Timer_1", data={"var1": "value1"}
        )
        ...
        # Restore timer
        restored_timer = await TimerEvent.restore("Timer_1", state, self.state_manager)
    ...
    async def test_cycle_timer_execution(self):
        ...
        timer = TimerEvent(
            "Timer_1", "R3/PT1S", self.state_manager
        )  # Repeat 3 times, 1 second each
        token = Token(instance_id="test-cycle-1", node_id="Timer_1", data={})
