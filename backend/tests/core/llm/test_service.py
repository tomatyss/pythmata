...
def mock_chat_response():
    ...
    mock_response.choices = [
        MagicMock(
            message=MagicMock(content="This is a test response"), finish_reason="stop"
        )
    ]
...
This XML defines a simple process with a start event, a task, and an end event."""
    ...
    mock_response.usage = MagicMock(input_tokens=150, output_tokens=100)
    return mock_response
...
async def test_chat_completion_model_format_conversion(llm_service, mock_chat_response):
    ...
    await llm_service.chat_completion(
        messages, model="anthropic/claude-3-7-sonnet-latest"
    )
