...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "value_data", postgresql.JSONB(astext_type=sa.Text()), nullable=False
        ),
    ...
    # Create indexes
    op.create_index(
        "ix_process_instances_definition_id", "process_instances", ["definition_id"]
    )
    op.create_index("ix_process_instances_status", "process_instances", ["status"])
    op.create_index("ix_scripts_process_def_id", "scripts", ["process_def_id"])
    op.create_index(
        "ix_script_executions_instance_id", "script_executions", ["instance_id"]
    )
    op.create_index("ix_variables_instance_id", "variables", ["instance_id"])
